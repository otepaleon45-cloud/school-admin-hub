...
const EMPTY = { name: "", telephone: "", email: "", access_code: "", salaire: "", periode: "mois" };
    ...
    <div data-testid="teachers-panel">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Enregistrez les enseignants et leur salaire (par mois ou par trimestre).</p>
        </div>
          ...
          <tbody>
            {teachers.map((t) => (
              <tr key={t.id} className="border-t border-slate-100" data-testid={`teacher-row-${t.id}`}>
                ...
                <td className="px-4 py-3 text-right font-mono text-slate-800">{money(t.salaire ?? t.salaire_trimestre)}</td>
                <td className="px-4 py-3 text-slate-600">{(t.periode || "trimestre") === "mois" ? "Par mois" : "Par trimestre"}</td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md bg-white">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block"><span className="text-xs font-medium text-slate-600">Salaire ($)</span><input type="number" min="0" className="fld mt-1" value={form.salaire} onChange={(e) => setForm({ ...form, salaire: e.target.value })} data-testid="input-teacher-salary" /></label>
            <label className="block"><span className="text-xs font-medium text-slate-600">Payé par</span>
              <select className="fld mt-1" value={form.periode} onChange={(e) => setForm({ ...form, periode: e.target.value })} data-testid="select-teacher-period"><option value="mois">Mois</option><option value="trimestre">Trimestre</option></select></label>
          </div>
          <DialogFooter>
